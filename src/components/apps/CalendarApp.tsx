...
export default function CalendarApp() {
  ...
  const [events, setEvents] = useLocalStorage('ubuntu-calendar-events', emptyEvents);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowRight') setCurrentMonth((prev) => prev.add(1, 'month'));
      if (e.key === 'ArrowLeft') setCurrentMonth((prev) => prev.subtract(1, 'month'));
    };
  ...
  const addOrUpdateEvent = () => {
    ...
    if (editingEventId) {
      setEvents((prev) =>
        prev.map((e) =>
          e.id === editingEventId ? { ...e, title: finalTitle, category: eventCategory } : e
        )
      );
    } else {
                ...
                return (
                  ...
                  >
                    ...
                    {dailyEvents.map((event) => (
                      ...
                      >
                        {formatTime(event.title, event.date)} – {event.title.replace(/\[.*?\]\s?/, '')}
                      </div>
                    ))}
                  </div>
                );
            ...
            {filteredEvents.length === 0 ? (
              ...
              Object.entries(
                filteredEvents
                  ...
                  .reduce((acc, event) => {
                    const dateKey = dayjs(event.date).format('dddd, DD MMM YYYY');
                ...
                <div key={dateLabel}>
                  <div className="text-ub-orange font-semibold text-base mb-2">{dateLabel}</div>
        ...
        {selectedDate && (
          ...
          >
            <h3 className="text-ub-orange text-sm mb-2 font-semibold">
              {editingEventId ? 'Edit' : 'Add'} Event – {dayjs(selectedDate).format('DD MMM YYYY')}
            </h3>
