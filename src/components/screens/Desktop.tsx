...
export default function DesktopScreen({
  ...
  const [folderCreationError, setFolderCreationError] = useState<string | null>(
    null
  );
  ...
  const [topOverlay, setTopOverlay] = useState<"apps" | "activities" | null>(
    null
  );
  ...
  const openOrFocusApp = (appId: string) => {
    ...
    if (
      existingWindow.workspaceIndex !== -1 &&
      existingWindow.workspaceIndex !== currentWorkspace
    ) {
    ...
    if (isMinimized[appId]) {
      window.dispatchEvent(
        new CustomEvent("restoreMinimized", { detail: appId })
      );
    }

    window.dispatchEvent(
      new CustomEvent("focusWindow", { detail: existingWindow.id })
    );
  };
  ...
  const handleCreateFolder = useCallback(
    (name?: string, rowId?: number) => {
      ...
      const normalizedId = `new-folder-${folderName
        .replace(/\s+/g, "-")
        .toLowerCase()}`;
      const existingFolders: { id: string; name: string }[] = JSON.parse(
        localStorage.getItem("new_folders") || "[]"
      );
      const alreadyExistsInApps = appsList.some(
        (app) => app.id === normalizedId
      );
      const alreadyExistsInStorage = existingFolders.some(
        (folder) => folder.id === normalizedId
      );
      ...
      const updatedFolders = [
        ...existingFolders,
        { id: normalizedId, name: folderName },
      ];
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("create-folder-from-terminal", handler);
  }, [handleCreateFolder]);
        ...
        screen:
          ...
          (() => (
            <div className="text-white p-4">
              No screen defined for {app.title}
            </div>
          )),
      };

      setWorkspaces((prev) =>
        prev.map((ws, i) => (i === currentWorkspace ? [...ws, newWindow] : ws))
      );
    };

    window.addEventListener("launchApp", handleLaunch as EventListener);
    return () =>
      window.removeEventListener("launchApp", handleLaunch as EventListener);
  ...
  useEffect(() => {
    const handleClose = (e: CustomEvent<string>) => {
      ...
      setWorkspaces((prev) =>
        prev.map((ws) =>
          ws.some((w) => w.id === id) ? ws.filter((w) => w.id !== id) : ws
        )
      );
    };
    ...
    return () =>
      window.removeEventListener("closeWindow", handleClose as EventListener);
  }, [currentWorkspace]);
  ...
  return (
    ...
    >
      <Navbar
        ...
        shutDown={shutDown}
        openActivities={toggleActivities}
      />
      ...
      <DefaultMenu active={iconMenuActive} position={iconMenuPosition} />
